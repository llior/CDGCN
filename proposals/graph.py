...
def graph_clustering_dynamic_th(edges,
    ...
    # there is more than one edge between nodes, so we need pool them
    if pool is None:
    ...
    while remain:
        th = th + (1 - th) * step
        max_sz = int(init_max_sz + th*0)
        comps, remain = connected_components_constraint(
            remain, max_sz, score_dict, th)
        th_list.append(th)
        clsNum_list.append(len(comps))
        clsNum_stastic_list.append(Counter([len(cls) for cls in comps]))
