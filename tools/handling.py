...
# modified : wangjie xmuspeech 2021.8.26
...
def main():
	...
	if args.heuristic:
		...
		for key in tqdm(files_list.reshape(-1)):
			print(key)
			...
			if matrix_rttm.shape[1] > 1 and os.stat("%s/%s.rttm" % (args.in_rttm_with_ov_dir, key)).st_size > 0:
				...
				#modefied by wangjie: to adapt the overlap rttm format of pyannote
				matrix_ov = np.sum(matrix_ov, axis=1)[:, np.newaxis]
