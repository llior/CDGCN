...
cohort_set_from=voxceleb1_train_aug_2fold # Should be a subset of $trainset if use cohort_set_method.
...
lda_process="submean-norm-trainlda"
whiten_process="submean-norm-lda-trainwhiten"
plda_process="submean-norm-lda-whiten-norm-trainplda"


enroll_process="submean-norm-lda-whiten-norm" # this process could have "mean" process for multi-utterance enroll.
test_process="submean-norm-lda--whiten-norm"

lda_data_config="$enrollset[$trainset $enrollset $testset]"
submean_data_config="$enrollset[$trainset $enrollset $testset]"
whiten_data_config="$enrollset[$trainset $enrollset $testset]"
...
for position in $positions;do
    ...
    for epoch in $epochs;do
        ...
        subtools/scoreSets.sh  --prefix $prefix --score $score --vectordir $obj_dir --enrollset $enrollset --testset $testset \
            --lda $lda --clda $clda --submean $submean --whiten $whiten --lda-process $lda_process --whiten_process $whiten_process \
            --trials $trials --extra-name "$extra_name" \
            --enroll-process $test_process --test-process $test_process --plda-process $plda_process \
            --lda-data-config "$lda_data_config" --submean-data-config "$submean_data_config" --whiten_data_config "$whiten_data_config" \
            --plda-trainset $trainset
