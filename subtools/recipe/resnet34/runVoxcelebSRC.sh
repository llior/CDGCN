...
voxceleb1_path=/data/zhd_data/Multimodal/voxceleb1/dev_test_audio
#voxceleb2_path=/data2/zhddata/Multimodal/data/voxceleb2
voxceleb2_path=/data/zhd_data/Multimodal/voxceleb2/dev/aac
rirs_path=/data2/zhddata/noise/RIRS_NOISES
musan_path=/data2/zhddata/noise/musan
...
#change the speed
subtools/kaldi/utils/perturb_data_dir_speed.sh 1.1 data/$prefix/voxceleb2_dev data/$prefix/voxceleb2_dev_speed1.1
subtools/kaldi/utils/data/perturb_data_dir_speed_3way.sh data/$prefix/voxceleb2_dev_plus_augx4 data/$prefix/voxceleb2_dev_plus_augx15
...
#                                 data/$prefix/voxceleb2_dev_spx3 data/$prefix/voxceleb2_dev_spx15
subtools/augmentDataByNoise.sh --rirs-noises $rirs_path --musan $musan_path --factor 4 \
...
subtools/makeFeatures.sh --nj 50 data/$prefix/voxceleb2_dev_plus_augx4 $feat_type $feat_conf
...
subtools/filterDataDir.sh --split-aug false data/$prefix/voxceleb2_dev_plus_augx4/ data/voxceleb2_dev/utt2spk \
                                            data/$prefix/voxceleb2_dev_clean_from_augx4
...
subtools/runPytorchLauncher.sh subtools/recipe/zhdresnet34/runResnet34-se-am.py --stage=0 --endstage=2

# [12] Train a thin Resnet34 model with AM-Softmax loss and 4 GPUs will be used to accelerate training
subtools/runPytorchLauncher.sh subtools/recipe/zhdresnet34/runResnet34-se-am.py --stage=3 --endstage=3 --gpu-id=0,1,2,3

# [13] Extract near xvectors in epoch 6 for voxceleb1 and voxceleb2_dev
subtools/runPytorchLauncher.sh subtools/recipe/zhdresnet34/runResnet34-se-am.py --stage=4
#xvectors-output = exp/resnet34_fbank_81_benchmark_n/near_epoch_6/voxceleb2_dev_clean_from_augx4/xvector.JOB.ark
### Back-end scoring
# [14] Score with submean + Cosine + AS-Norm processes
tasks="vox1-O vox1-O-clean vox1-E vox1-E-clean vox1-H vox1-H-clean"
#tasks=" vox1-E vox1-E-clean vox1-H vox1-H-clean"
score_norm=false
for task in $tasks;do
    [ "$task" == "vox1-O" ] && score_norm=true
    [ "$task" == "vox1-O-clean" ] && score_norm=true
    subtools/recipe/zhdresnet34/scoreSets_asnorm.sh --prefix $prefix --score cosine  --submean true \
         --vectordir "exp/512D_resnet34_fbank_81_benchmark_n" --task $task --epochs "5" --positions "near" \
         --score-norm $score_norm --score-norm-method "asnorm" --top-n 100 --cohort-set voxceleb2_dev_clean_from_augx4
done
