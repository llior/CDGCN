...
voxceleb1_path=/data2/zhddata/Multimodal/data/voxceleb1
...
#change the speed
subtools/kaldi/utils/perturb_data_dir_speed.sh 1.1 data/$prefix/voxceleb2_dev data/$prefix/voxceleb2_dev_sp1.1
subtools/kaldi/utils/data/perturb_data_dir_speed_3way.sh data/voxceleb2_dev data/$prefix/voxceleb2_dev_spx2
...
subtools/runPytorchLauncher.sh subtools/pytorch/launcher/runEcapaXvector.py --stage=0 --endstage=2

# [12] Train a thin Resnet34 model with AM-Softmax loss and 4 GPUs will be used to accelerate training
subtools/runPytorchLauncher.sh subtools/pytorch/launcher/runEcapaXvector.py --stage=3 --endstage=3 --gpu-id=2

# [13] Extract near xvectors in epoch 6 for voxceleb1 and voxceleb2_dev
subtools/runPytorchLauncher.sh subtools/pytorch/launcher/runEcapaXvector.py --stage=4
...
for task in $tasks;do
    [ "$task" == "vox1-O" ] && score_norm=false
    [ "$task" == "vox1-O-clean" ] && score_norm=false
    subtools/recipe/zhdresnet34/scoreSets_asnorm.sh --prefix $prefix --score cosine  --submean false \
         --vectordir "exp/ecapa-tdnn-xvector_Tao" --task $task --epochs "6" --positions "near" \
