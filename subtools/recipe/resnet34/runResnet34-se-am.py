...
model_params = { 
    ...
    "resnet_params":{
            ...
            "block":"BasicBlock",
            "layers":[3, 4, 6, 3],
            "planes":[64, 128, 256, 512],  #原来的"planes":[32, 64, 128, 256],为了和visual的embedding的512-D的embedding对齐
            "convXd":2,
...
##--------------------------------------------------##
## Other options
# exist_model="/tsdata/kaldi/egs/tfc/gan/exp/vox2augx4spx3_fbank81_sequential/resnet34_am/4.params"  # Use it in transfer learning.
exist_model=""
##--------------------------------------------------##
## Main params
traindata="data/fbank_81/voxceleb2_dev_plus_augx4"
egs_dir="exp/egs/fbank81_voxceleb2_dev_augx4_sequential"
#egs_dir="exp/egs/512D_fbank81_voxceleb2_dev_augx4_sequential"



# egs_dir="exp/egs/fbank81_voxceleb2_train_augx4_spx3_sequential"
# egs_dir="exp/egs/fbank_81_voxceleb1_train_aug_2fold_sequential"
# egs_dir="/tsdata/kaldi/egs/tfc/gan/exp/egs/fbank81_vox1x2_vox2_sequential"
# egs_dir="/tsdata/kaldi/egs/tfc/gan/exp/egs/fbank81_vox1x2_vox2_dgsn_sequential"
# egs_dir="exp/egs/fbank81_voxceleb1_test_sequential"

model_blueprint="subtools/pytorch/model/resnet-se-xvector.py"
model_dir="exp/512D_resnet34_fbank_81_benchmark_n"
# model_dir="exp/resnet34_fbank_81_benchmark_n"
# model_dir="exp/vox2augx4spx3_fbank81_sequential/resnet34_se_am"
# model_dir="exp/vox1augx1_fbank81_sequential/resnet34_se_am"
# model_dir="exp/vox1augx1_fbank81_sequential/vox1_vox2_resnet34_se_am"
# model_dir="exp/vox1augx1_fbank81_sequential/vox1_vox2_dgcn_resnet34_se_am"
# model_dir="exp/vox2spd3x2+4_fbank81_sequential/resnet34_am"
##--------------------------------------------------##
##
######################################################### START #########################################################
##
...
#     #to_extracted_data = ["voxceleb2_dev_plus_augx4"] # All dataset should be in data_root/prefix.
    
#     to_extracted_data = ["voxceleb2_dev_clean_from_augx4"]
...
#             # model_dir="exp/resnet34_fbank_81_benchmark_n"
#             model_blueprint, model_creation = utils.read_nnet_config("{0}/config/nnet.config".format(model_dir)) #读取模型和相关配置
...
#             utils.write_nnet_config(model_blueprint, model_creation, "{0}/config/{1}".format(model_dir, extract_config)) # 把配置写入 
#             for epoch in to_extracted_epochs:
#                 model_file = "{0}.{1}".format(epoch, suffix)#suffix = "params"模型参数
...
#                     datadir = "{0}/{1}/{2}".format(data_root, prefix, data)# data_root = "data", prefix = "fbank_81", data = ["voxceleb2_dev_clean_from_augx4"]
#                     outdir = "{0}/{1}/{2}".format(model_dir, point_name, data) # exp/resnet34_fbank_81_benchmark_n/near_epoch_6/voxceleb2_dev_clean_from_augx4
...
#                                                 " --force {force} --nnet-config config/{extract_config} --vad {vad} "
#                                                 "{model_dir} {datadir} {outdir}".format(model_file=model_file, cmn=str(cmn).lower(), nj=nj,
#                                                 use_gpu=str(use_gpu).lower(), gpu_id=gpu_id, force=str(force).lower(), extract_config=extract_config,vad=str(vad).lower(),
...
if stage <= 4 <= endstage and utils.is_main_training():
    ...
    to_extracted_positions = ["near"] # Define this w.r.t extracted_embedding param of model_blueprint.
    #to_extracted_data = ["voxceleb2_dev_plus_augx4"] # All dataset should be in data_root/prefix.
    to_extracted_data = ["voxceleb2_dev"]  #测试集
    #to_extracted_data = ["voxceleb2_dev_clean_from_augx4"]   #如果要计算asnorm上面这三个都要
    to_extracted_epochs = [5] # It is model's name, such as 10.params or final.params (suffix is w.r.t package).
        ...
        for position in to_extracted_positions:
            ...
            for epoch in to_extracted_epochs:
                ...
                for data in to_extracted_data:
                    ...
                    datadir = "{0}/{1}/{2}".format(data_root, prefix, data)# data_root = "data", prefix = "fbank_81", data = ["voxceleb2_dev_clean_from_augx4"]
                    outdir = "{0}/{1}/{2}".format(model_dir, point_name, data) # exp/resnet34_fbank_81_benchmark_n/near_epoch_6/voxceleb2_dev_clean_from_augx4
                    #data1 = "voxceleb2_audio_visual"
                    #data1 = "voxceleb1"
                    #outdir = "{0}/{1}/{2}".format(model_dir, point_name, data1) # data1 = "voxceleb2_audio_visual or voxceleb1"
                    # 原来的命令：subtools/pytorch/pipeline/extract_xvectors_for_pytorch.sh没有另外提取embedding单独保存  extract_audio_visual_xvectors_for_pytorch.sh
                    # 下面这个命令可以将vox2的embedding保存到相应的所有子文件中 
                    # Use a well-optimized shell script (with multi-processes) to extract xvectors.
                    # Another way: use subtools/splitDataByLength.sh and subtools/pytorch/pipeline/onestep/extract_embeddings.py 
                    # with python's threads to extract xvectors directly, but the shell script is more convenient.
                    kaldi_common.execute_command("bash subtools/pytorch/pipeline/extract_audio_visual_xvectors_for_pytorch.sh "
                                                "--model {model_file} --cmn {cmn} --nj {nj} --use-gpu {use_gpu} --gpu-id '{gpu_id}' "
                                                " --force {force} --nnet-config config/{extract_config} --vad {vad} "
                                                "{model_dir} {datadir} {outdir}".format(model_file=model_file, cmn=str(cmn).lower(), nj=nj,
                                                use_gpu=str(use_gpu).lower(), gpu_id=gpu_id, force=str(force).lower(), extract_config=extract_config,vad=str(vad).lower(),
