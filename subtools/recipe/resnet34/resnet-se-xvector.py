...
class ResNetXvector(TopVirtualNnet):
    ...
    def init(self, inputs_dim, num_targets, aug_dropout=0., tail_dropout=0., training=True, extracted_embedding="near", 
        ...
        if pooling == "lde":
            ...
            self.stats = AttentiveStatisticsPooling(resnet_output_dim, affine_layers=pooling_params["affine_layers"], 
            ...
            self.stats = xivec_stdinit_softplus2_prec_pooling(resnet_output_dim, hidden_size=pooling_params["hidden_size"], stddev=False)
        elif pooling == "xi-postdist-softplus2":
            self.stats = xivec_stdinit_softplus2_prec_pooling(resnet_output_dim, hidden_size=pooling_params["hidden_size"], stddev=True) 
        elif pooling == "statistics":
            print("using stats pooling")
