...
epochs="1"
positions="near"
vectordir="exp/resnet34_fbank_81_benchmark_n"

task="vox1-O-clean"
trials=
srcdata=voxceleb1
trainset=voxceleb2_dev_plus_augx4
enrollset=                #voxsrc2020_enroll  #由subtools/split_enroll_test_by_trials.sh得到
testset=                 #voxsrc2020_test
...
top_n=100
cohort_set=voxceleb2_dev_clean_from_augx4 # If not NULL, use provided set
score_norm_spk_mean=true

cohort_method="sub" # "sub" | "mean". It works when cohort_set is empty.
cohort_set_from=voxceleb2_dev_plus_augx4 # Should be a subset of $trainset if use cohort_set_method.
...
for position in $positions;do
    ...
    for epoch in $epochs;do
        obj_dir=$vectordir/${position}_epoch_${epoch}
                                      # obj_dir = exp/resnet34_fbank_81_benchmark_n/near_epoch_6/       enrollset=${task_name}_enroll && testset=${task_name}_test
        # Prepare task for scoring. Here it is only needed to extract voxceleb1 xvectors and then it will split subsets.
        # voxceleb -> voxceleb1-O/E/H[-clean]_enroll/test  ######prefix=fbank_81   srcdata=voxceleb1  prepare_trials=data/$prefix/$srcdata/voxceleb1-O.trials;
        if [ "$split" == "true" ];then
        ...
        if [[ "$score_norm" == "true" && -f $obj_dir/$name.score ]];then
            ...
            subtools/scoreSets.sh  --prefix $prefix --eval true --score $score --vectordir $obj_dir \
                ...
                --trials data/$prefix/$cohort_set/$enrollset.cohort.trials 
            ...
            subtools/scoreSets.sh  --prefix $prefix --eval true --score $score --vectordir $obj_dir \
                ...
                --trials data/$prefix/$cohort_set/$testset.cohort.trials 
            ...
            mindcf1=`subtools/kaldi/sid/compute_min_dcf.py --p-target 0.01 $obj_dir/$output_name.score $trials 2> /dev/null`
            mindcf2=`subtools/kaldi/sid/compute_min_dcf.py --p-target 0.001 $obj_dir/$output_name.score $trials 2> /dev/null`
            echo "AS-Norm minDCF(p-target=0.01): $mindcf1"
            echo "AS-Norm minDCF(p-target=0.001): $mindcf2"
