...
if [[ "$stage" -le 3 && "$endstage" -ge "$stage" ]];then
	...
	echo "NCCL_P2P_LEVEL=0 CUDA_VISIBLE_DEVICES=0,1,2,3 $train_cmd $launcher $launcher_options --stage=$stage --endstage=$endstage --force=true"
	NCCL_P2P_LEVEL=NVL NCCL_BLOCKING_WAIT=1 $train_cmd $launcher $launcher_options --stage=$stage --endstage=$endstage --force=true|| exit 1 
fi
