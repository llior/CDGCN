...
# JFZhou 2020-04-26
def read_vec(file,num_vecs=None):
    if num_vecs:
        num_read = 0
        try:
            for key,vector in read_vec_flt_ark(file):
                num_read+=1
                yield key,vector
                if num_read>=num_vecs:
                    break
        except:
            try:
                for key,vector in read_vec_flt_scp(file):
                    num_read+=1
                    yield key,vector
                    if num_read>=num_vecs:
                        break
            except:
                print('Error:The file type of '+file+' is not accepted,the file must be a ark file or scp file')
                exit()
    else:
        try:
            for key,vector in read_vec_flt_ark(file):
