...
subtools/kaldi/utils/data/get_utt2dur.sh ${srcdir}
subtools/kaldi/utils/data/get_reco2dur.sh ${srcdir}

subtools/kaldi/utils/data/perturb_data_dir_speed.sh 1.3 ${srcdir} ${destdir}_speed1.3 || exit 1
subtools/kaldi/utils/data/perturb_data_dir_speed.sh 1.4 ${srcdir} ${destdir}_speed1.4 || exit 1
subtools/kaldi/utils/data/perturb_data_dir_speed.sh 1.5 ${srcdir} ${destdir}_speed1.5 || exit 1
subtools/kaldi/utils/data/perturb_data_dir_speed.sh 0.7 ${srcdir} ${destdir}_speed0.7 || exit 1
subtools/kaldi/utils/data/perturb_data_dir_speed.sh 0.6 ${srcdir} ${destdir}_speed0.6 || exit 1
subtools/kaldi/utils/data/perturb_data_dir_speed.sh 0.5 ${srcdir} ${destdir}_speed0.5 || exit 1

exit 1;
if $always_include_prefix; then
  subtools/kaldi/utils/copy_data_dir.sh --spk-prefix sp1.0- --utt-prefix sp1.0- ${srcdir} ${destdir}_speed1.0
  ...
  subtools/kaldi/utils/data/combine_data.sh $destdir ${destdir}_speed1.0 ${destdir}_speed0.9 ${destdir}_speed1.1 ${destdir}_speed0.8 ${destdir}_speed1.2 || exit 1

  # rm -r ${destdir}_speed0.9 ${destdir}_speed1.1 ${destdir}_speed1.0 ${destdir}_speed0.8 ${destdir}_speed0.8
else
  subtools/kaldi/utils/data/combine_data.sh $destdir ${srcdir} ${destdir}_speed0.9 ${destdir}_speed1.1 ${destdir}_speed0.8 ${destdir}_speed1.2 || exit 1
  # rm -r ${destdir}_speed0.9 ${destdir}_speed1.1 
fi

echo "$0: generated 3-way speed-perturbed version of data in $srcdir, in $destdir"
if ! subtools/kaldi/utils/validate_data_dir.sh --no-feats --no-text $destdir; then
