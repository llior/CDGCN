...
class DAL_regularizer(TopVirtualNnet):
    ...
    def init(self, n_in):
        ...
        self.w_id1 = nn.Linear(n_in, 1, bias=False)
        self.w_noise1 = nn.Linear(n_in, 1, bias=False)
    
    def forward(self, features_noise, features_id):
        features_noise1 = self.w_noise(features_noise)
        features_id1 = self.w_id(features_id)
        normalized_id = F.normalize(features_id1.squeeze(dim=2), dim=1)
        normalized_noise = F.normalize(features_noise1.squeeze(dim=2), dim=1)
        cosine = torch.sum(normalized_id * normalized_noise, dim=1).mean()

        vs_id = self.w_id1(features_id.squeeze())
        vs_noise = self.w_noise1(features_noise.squeeze())
        rho = ((vs_noise - vs_noise.mean(dim=0)) * (vs_id - vs_id.mean(dim=0))).mean(dim=0).pow(2) \
                / ( (vs_noise.var(dim=0) + 1e-6) * (vs_id.var(dim=0) + 1e-6))
