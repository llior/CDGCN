...
class ECAPA_TDNN(TopVirtualNnet):
    def init(self, inputs_dim, num_targets, channels=512, embd_dim=192, 
        ...
        if pooling == "attentive":
            ...
            self.stats = xivec_stdinit_softplus2_prec_pooling(cat_channels, hidden_size=pooling_params["hidden_size"], stddev=False)
            self.bn_stats = nn.BatchNorm1d(cat_channels * 1)
            self.fc1 = ReluBatchNormTdnnLayer(cat_channels * 1, embd_dim, **fc1_params) if fc1 else None
        elif pooling == "xi-postdist-softplus2":
            self.stats = xivec_stdinit_softplus2_prec_pooling(cat_channels, hidden_size=pooling_params["hidden_size"], stddev=True)
            self.bn_stats = nn.BatchNorm1d(cat_channels * 2)
            self.fc1 = ReluBatchNormTdnnLayer(cat_channels * 2, embd_dim, **fc1_params) if fc1 else None
        else:
