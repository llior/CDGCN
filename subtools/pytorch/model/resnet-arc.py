...
class ResNetXvector(TopVirtualNnet):
    ...
    def init(self, inputs_dim, num_targets, aug_dropout=0., tail_dropout=0., training=True, extracted_embedding="near", 
             ...
             use_step=False, step_params={}, sub_Arc_adaptive=False,Arc_adaptive=False,margin=0.2,
        ...
        if training :
            ...
            if sub_Arc_adaptive:
                self.loss = sub_ArcFaceLossAdaptiveMargin(resnet_params["planes"][3],num_targets,margin)
            if Arc_adaptive:
                self.loss = ArcFaceLossAdaptiveMargin(resnet_params["planes"][3],num_targets,margin)
            else:
                self.loss = SoftmaxLoss(resnet_params["planes"][3], num_targets)

            # An example to using transform-learning without initializing loss.affine parameters
            # self.transform_keys = ["resnet", "stats", "fc1", "fc2"]
            self.transform_keys = ["resnet", "stats"]
