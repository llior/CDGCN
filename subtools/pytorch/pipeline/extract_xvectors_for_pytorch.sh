...
#                     (Author: wangjie  2020-03-13 "Add subsegments file")
nj=30
cmd="run.pl"
stage=1
cmn=true
...
force=true
model=6.params
nnet_config=config/near.extract.config 
srcdir=exp/Resnet_34_se_am_train_cnceleb_16k_fbank81/
data=data/$name 
dir=exp/Resnet_34_se_am_train_cnceleb_16k_fbank81/$name
```
...
# Set up sliding-window subsegments
  ...
  if [ -f $data/subsegments ];then
	echo "subsegments already exist! use it"
	\cp $data/subsegments $dir/
  ...
  echo "create subsegments file"
  fix_data_dir.sh $data
  subtools/kaldi/utils/data/subsegment_data_dir.sh $data \
      $dir/subsegments $sub_data
  echo "finish the subsegments"
# Creat visual vad
subtools/createVisualVad.sh --nj $nj $sub_data
data=$sub_data
fi
