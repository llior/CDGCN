...
class PlotTrainingCSV():
    def __init__(self, csv_path, is_cmp=False):
        ...
        # self.train_acc_sample = list(self.df['train_acc'])
        self.train_acc = list(self.df[pd.isna(self.df['valid_acc']) == False]['train_acc'])
        if is_cmp==False:
    ...
    def smooth_valid_acc(self, beta=0.9):
        proto_train_acc= self.valid_acc
        smooth_train_acc = []
        prosum = 0.0
        for i in range(0, len(proto_train_acc)):
            if(i == 0):
            ...
            prosum = prosum * beta + (1 - beta) * proto_train_acc[i]
            smooth_train_acc.append(prosum)
        self.valid_acc = smooth_train_acc
        # self.df['valid_acc'] = smooth_train_acc
    ...
    def draw_acc(self, row=2, col=3, no=1, x_index=3, x_maj=30000, x_min=10000, is_cmp=False):
        if is_cmp==False:
            fig = self.mainfig.add_subplot(row, col, no)
        ...
        l_train_acc, = plt.plot(x_valid_acc, self.train_acc, color='red', label='Train set')
        
        l_valid_acc, = plt.plot(x_valid_acc, self.valid_acc, color='green', label='Validation set')
        ...
        plt.plot(pd.core.series.Series(x_train_last), pd.core.series.Series(y_train_last), 'x', label='Train set (epoch)')
        plt.plot(pd.core.series.Series(x_valid_last), pd.core.series.Series(y_valid_last), 'x', label='Validation set (epoch)')

        if is_cmp==False:
            x_size = 10 ** x_index
            def formatnum(x, pos):
                # return '${%d}$x$10^{%d}$' % (x/x_size, x_index)
                return '${%d}$K' % (x/x_size)
            ...
            y_min_locator=MultipleLocator(10)
            y_major_locator=MultipleLocator(20)
            fig.xaxis.set_minor_locator(xminorLocator)
            fig.xaxis.set_major_locator(xmajorLocator)
            fig.yaxis.set_minor_locator(y_min_locator)
            fig.yaxis.set_major_locator(y_major_locator)
            # fig.xaxis.get_major_formatter().set_powerlimits((0,1))

        plt.xlabel('Iterations')
        plt.ylabel('Accuracy (%)')
        plt.title('Label Noise Proportion - 0%')
        plt.grid()
        plt.ylim(-4,104)
        plt.legend(loc=4)
...
if __name__ == "__main__":
    figure=PlotTrainingCSV("/tsdata/kaldi/egs/tfc/gan/exp/sre04-10_SWBD_mfcc40_sre_sequential_noisy_labels/extended_am_error0/log/train.csv")
    figure.smooth_acc(beta=0.8)
    figure.smooth_valid_acc(beta=0.8)
    figure.draw_acc(no=2)
    # figure=PlotTrainingCSV("/tsdata/kaldi/egs/tfc/gan/exp/sre046810_swbd_mfcc40_8k_sequential_noisy_label/extended_am_sgd_error50/log/train.csv")
    # figure.smooth_acc(beta=0.7)
    # figure.smooth_valid_acc(beta=0.7)
    # figure.draw_acc(no=2)
