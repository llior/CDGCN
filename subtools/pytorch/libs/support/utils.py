...
def get_adapt_margin_list(path):
    train_csv = pd.read_csv(path+"/train.egs.csv",header=None,sep=" ",low_memory=False)
    tmp = np.sqrt(1 / np.sqrt(train_csv[4].value_counts().sort_index().values))
    margin = (tmp - tmp.min()) / (tmp.max() - tmp.min()) * 0.45 + 0.05
    return margin