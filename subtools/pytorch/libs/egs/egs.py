...
class ChunkEgs(Dataset):
    ...
    """
    def __init__(self, egs_csv, egs_type="chunk", io_status=True,two_label=False, aug=None, aug_params={}):
        """
        @egs_csv:
        ...
        # self.utt_id = pd.read_csv(egs_csv, sep=" ", usecols = ["utt-id"]).values.astype(np.string_)
        self.label = pd.read_csv(egs_csv, sep=" ", usecols = ["class-label"]).values
        if self.two_label:
            self.label2 = pd.read_csv(egs_csv, sep=" ", usecols = ["4"]).values # 4 means the second label head in the csv
    ...
    def __getitem__(self, index):
        ...
        if self.aug is not None:
            ...
            if self.two_label:
                ...
                return self.aug(egs.T), target,target2
            else:
...
class BaseBunch():
    ...
    def get_bunch_from_csv(self, trainset_csv:str, valid_csv:str=None, egs_params:dict={}, data_loader_params_dict:dict={}):
        ...
        if "two_label" in egs_params.keys():
            two_label = egs_params["two_label"]
        else:
            two_label = False
        ...
        if valid_csv != "" and valid_csv is not None:
            if two_label:
                valid = ChunkEgs(valid_csv, two_label=True, egs_type=egs_type)
            else:
