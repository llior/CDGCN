...
class ChunkDropout(torch.nn.Module):

    def __init__(self,multi_times=10,max_dropout_frames=10):
        super(ChunkDropout, self).__init__()
    ...
    def __call__(self, inputs):
        ...
        for i in range(self.multi_times):
            num_t = inputs.size(2)
#         print("num_t",num_t)
            t = np.random.randint(0, self.max_dropout_frames + 1)
            t_0 = np.random.randint(0, num_t - t + 1)
        # print("t_0",t_0)
            out1 = inputs[:,:,:t_0]
            out2 = inputs[:,:,t_0+t:]
            inputs = torch.cat([out1,out2],2)
        # input_size = inputs.shape
        # if not self.training: return inputs
        # if self.max_dropout_frames > 0 :
        #     assert input_size[2]-self.max_dropout_frames > 100
        #     dropout_frames = int(random.randint(0,self.max_dropout_frames)/2)

        #     new_chunk_size = input_size[2]-dropout_frames
        #     outputs = inputs[:,:,dropout_frames:new_chunk_size]
