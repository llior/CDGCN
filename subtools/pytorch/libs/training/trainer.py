...
class GanDalAttTrainer(_BaseTrainer):
    """One input and one output.
    """
    def __init__(self, *args, **kwargs):
        super(GanDalAttTrainer, self).__init__(*args, **kwargs)
        self.trainingDAL = False
        self.model = self.elements["model"]

    def set_train_mode(self, model,state):

      
        self.trainingDAL = not state

        self.set_grads(model.tdnn1,state)
        self.set_grads(model.tdnn2,state)
        self.set_grads(model.tdnn3,state)
        self.set_grads(model.tdnn4,state)
        self.set_grads(model.tdnn5,state)
        self.set_grads(model.tdnn6,state)
        self.set_grads(model.tdnn7,state)
        self.set_grads(model.ex_tdnn1,state)
        self.set_grads(model.ex_tdnn2,state)
        self.set_grads(model.ex_tdnn3,state)
        self.set_grads(model.ex_tdnn4,state)
        self.set_grads(model.ex_tdnn5,state)
        self.set_grads(model.stats,state)
        self.set_grads(model.loss,state)
        self.set_grads(model.loss2,state)
        self.set_grads(model.att,state)
        self.set_grads(model.RFM,state)

        self.set_grads(model.DAL, not state)
    ...
    def train_one_batch(self, batch,i):
        """A normal training core without fetching data from iterator.
        """
        model = self.elements["model"]
        model_forward = self.elements["model_forward"]
        optimizer = self.elements["optimizer"]
        optimizer2 = optim.SGD(model.DAL.parameters(), lr=0.01, momentum=0.9,weight_decay=1e-4)
        # optimizer2 = optim.Adam(model.DAL.parameters(), lr=0.001, betas=(0.9, 0.999), weight_decay=1e-4)

        if not model.training:
            model.train()
        if i % 70 == 0: # canonical maximization procesure
            self.set_train_mode(model,False)
        elif i % 70 == 20: #  RFM optimize procesure
            self.set_train_mode(model,True)
        ...
        # print(targets2)
        # exit()
        model_output = model_forward(inputs)
        # print(model_output[2])
        # exit()
        # print(model_output[0].shape)
        # print(targets.type)
        # exit()
        loss = model.get_loss(model_output[0], targets,model_output[1],targets2) + 0.1 * model_output[2]

        # loss = model.get_loss(model_output[0], targets,model_output[1],targets2)
        if self.trainingDAL:
            optimizer2.zero_grad()
            loss.backward()
            self.flip_grads(model.DAL)
            loss.detach() # For safe.
            optimizer2.step()
        else:
            optimizer.zero_grad()
        ...
        accuracy = model.get_accuracy(targets) if self.params["compute_accuracy"] else None
        cc = model_output[2]
        return loss.item(), accuracy,cc.item()
    ...
    def compute_validation(self, data_loader):
        ...
        with torch.no_grad():
            for this_data in data_loader:
                inputs, targets, targets2 = this_data
                targets = targets.long()
                targets2 = targets2.long()
                model_output = model_forward(inputs)
                loss += model.get_loss(model_output[0], targets,model_output[1],targets2).item() * len(targets)
                num_samples += len(targets)
        ...
        try:
            ...
            for this_epoch in range(start_epoch, epochs):
                ...
                for this_iter, batch in enumerate(data.train_loader, 0):
                    ...
                    loss, acc,cc = self.train_one_batch(batch,this_iter)
