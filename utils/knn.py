...
__all__ = [
    'knn_brute_force', 'knn_hnsw', 'knn_faiss_online', 'knn_faiss', 'knn_faiss_gpu', 'knns2spmat',
...
#         knns = np.array(knns)
#     if not sort:
#         return knns[:, 0, :].astype(np.int32), knns[:, 1, :]
#     else:
#         # sort dists from low to high
#         idxs = np.arange(knns.shape[0]).reshape(-1, 1)
#         return knns[:, 1, :][idxs, np.argsort(knns[:, 1, :], axis=1)], knns[:, 0, :].astype(np.int32)[idxs, np.argsort(knns[:, 1, :], axis=1)]
...
class knn_faiss_online(knn):
    def __init__(self,
        ...
        if feats.shape[0]<k:
            k = feats.shape[0]
            print("Warning:the shape of feats is {}, but got k is {}. Decrease the k to {}".format(feats.shape, k, feats.shape[0]))
        import faiss
        if omp_num_threads is not None:
            faiss.omp_set_num_threads(omp_num_threads)
        self.verbose = verbose
...
#add by wangjie
#param: if is_precise = False, dist will be got
def build_knns(knn_prefix,
               feats,
               knn_method,
               k,
