...
def Link_cluster_infomap(edges, scores, min_sim=0):
    """
    基于infomap的聚类
    :param nbrs:
    :param dists:
    ...
    :return:
    """
    total_node = set([ node for edge in edges for node in edge])
    infomapWrapper = infomap.Infomap("--two-level --directed")
    for node_pair, sim in tqdm(tzip(edges, scores)):
        if sim<min_sim: continue
        if node_pair[0] in total_node: total_node.remove(node_pair[0])
        if node_pair[1] in total_node: total_node.remove(node_pair[1])
        _ = infomapWrapper.addLink(node_pair[0], node_pair[1], sim)

    single = list(total_node)
    ...
    clusters = list(label2idx.values())
    return clusters
